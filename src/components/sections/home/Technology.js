import TeamCard3 from "@/components/shared/cards/TeamCard3";
import getTeamMembers from "@/libs/getTeamMembers";
import Link from "next/link";
import getTechnology from "@/libs/getTechnology";
import TechCard from "@/components/shared/cards/TechCard";

const Technology = () => {
	const items = getTechnology();
	return (
		<section className="h8-team section-gap section-gap-x">
			<div className="container">
				<div className="row  h8-team-wrapper  gap-0">
					<div className="col-lg-3 col-sm-6 ">
						<div className="sec-heading style-3">
							<span className="sub-title wow fadeInUp" data-wow-delay=".3s">
								MEET OUR TEAM
							</span>
							<h2 className="sec-title title-anim">
								Meet the Mind Behind Our Success.
							</h2>
						</div>
					</div>
					{items?.length
						? items.map((item, idx) => (
								<TechCard key={idx} teamMember={item} idx={idx} />
						  ))
						: ""}

				</div>
			</div>
			<div className="bg-shape-1">
				<img src="/images/shape/pattern-2.svg" alt="" />
			</div>
			<div className="bg-shape-2">
				<img src="/images/shape/pattern-3.svg" alt="" />
			</div>
		</section>
	);
};

export default Technology;
